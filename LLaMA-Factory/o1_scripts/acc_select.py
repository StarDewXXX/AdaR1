...
def generate_loft_dataset(data):
    ...
    for problem_index in tqdm(range(num_problems)):
        ...
        for index in range(0, count_per_problem):#random.sample(range(0, len(solutions)), count_per_problem):
            ...
            acc_term = int(correctness[index]) - avg_acc
            weight = acc_term
            
            # print(f"length:{length} acc:{correctness[index]}, avg_acc:{avg_acc}, weight:{weight}")
            selected_items.append(
                {
                    "problem": problem,
                    "solution": solution,
    ...
    save_dir = f"../LLaMA-Factory/data/my_dataset/{model_id}-iter0-MATH-train-K-16-acc"
    os.makedirs(save_dir, exist_ok=True)
    filename = f"{save_dir}/raw.json"
    with open(filename,"w") as f:
        json.dump(dataset_data,f)
