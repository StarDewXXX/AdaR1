...
for K_value in K_values:
    ...
    for problem_index in tqdm(range(num_problems)):
        ...
        solutions = [item['solution'] for item in items][0:K_value]
        answers = [item['answer'] for item in items][0:K_value]
        ...
        num_valid_answer = sum([int(answer == "None") for answer in answers])
        ...
        if len(sorted_valid_answer_groups) > 0:
            ...
            if grade_answer(sorted_valid_answer_groups[0][0], real_answer):
                ...
                # print(correct_solutions[max_index])
                # print("min:",min(lengths),"max:",max(lengths),"avg:",(sum(lengths)/len(lengths)))
                # input("continue?")
    
    print(f"correct ratio:{correct/num_problems} avg_tokens:{total_tokens/(K_value*num_problems)}")
print(file_name)