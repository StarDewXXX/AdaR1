...
def get_gated_batch_logps(
    logits: "torch.Tensor", labels: "torch.Tensor", label_pad_token_id: int = IGNORE_INDEX, combined_mask=None, gating_threshold=100
) -> Tuple["torch.Tensor", "torch.Tensor"]:
    r"""
    Computes the gated log probabilities of the given labels under the given logits,
    ignoring positions where probabilities are greater than gating_threshold.
    ...
    with torch.no_grad():
        ...
        high_conf_mask = per_token_probs <= gating_threshold  # Mask positions with prob > gating_threshold

    per_token_logps = logits.log_softmax(-1).gather(dim=2, index=labels.unsqueeze(2)).squeeze(2)

    # Combine masks: ignore both padding and high confidence positions

    if combined_mask is None:
        combined_mask = loss_mask & high_conf_mask
    print("loss_mask:",loss_mask.sum(-1),"combined_mask:",combined_mask.sum(-1))

    return (per_token_logps * combined_mask).sum(-1), combined_mask
